...
public class EmblemBadgesManager {
    ...
    public List<BadgeAttribute> getBoosts(Emblem emblem, LeagueBadgesManager badgesInventories){
        ...
        for (EmblemTargetItem item : targets) {
            ...
            BadgeSlot badgeSlot = league.getBadgeFromItem(targetContainer.getItem(item.targetSlot()));
            boosts.add(badgeSlot.getAttribute(emblem.getSlot(item.currentSlot()).category()));
        }
        return boosts;
    }
